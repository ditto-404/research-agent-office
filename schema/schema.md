...
**이름 규칙** - `Journal Watch`는 팀 **종류**의 이름이고(`teams.json`의 `type` 값
`journal-watch`), 개별 팀의 id·표시 이름은 그와 별개로 자유롭게 정한다. 팀 자체를
가리킬 때는 그 팀의 표시 이름을 쓰고, "Journal Watch"는 종류를 가리킬 때만 쓴다.
타입 판별은 항상 id가 아니라 `type` 필드로 한다.
...
- `raw/{slug}.json` (위치: `data/`) - 논문의 원본 메타데이터. API/웹에서 그대로
  ...
  참고. 사용자가 PDF나 (PDF에서 텍스트를 추출한) `.md`를 직접 구해서 `raw/`에
  넣으면(파일명은 아무거나 상관없다, `/paper-review-pdf`가 원문에서 제목/연도를
  읽어 표준 slug로 리네임한다) `/paper-review-pdf`로 전문 기반 반영한다.
- `wanted.md` (위치: `<WIKI_VAULT_ROOT>/<team-id>/`) - Paper Collector가 검색은
  했지만 전문을 확보하지 못한 논문 중, 이 팀의 연구와 관련성이 높아 보이는 것만 골라
  올려두는 목록. raw나 wiki papers로 반영되지는 않은, "전문을 구하면 좋을 후보"
  목록이다. 사용자가 전문(PDF/`.md`)을 구해서 `/paper-review-pdf`로 반영하면 이
  목록에서 빠진다.
...
- `journals`가 채워져 있으면 그 목록을 그대로 우선 사용하고, 위 저명도 판단은 목록에
  없는 추가 후보를 찾을 때만 보조적으로 쓴다. **Journal Watch(`journals`가 있는
  경우)는 특정 주제/키워드로 검색을 좁히지 않는다** - 각 저널의 최근호(latest
  issue)나 TOC를 훑어서 그 저널이 최근에 실제로 무엇을 실었는지 폭넓게 확인하는
  것이 목표다. `field`는 참고일 뿐, 검색 쿼리에 키워드로 그대로 넣어 결과를 좁히면
  그 저널의 "최근 동향"이 아니라 "그 키워드가 들어간 논문만" 보게 된다. **이
  "키워드로 좁히지 않기"는 Journal Watch 전용이다 - Research Team은 아래 항목대로
  여전히 자기 `topic`/`keywords`로 검색한다.**
- Research Team의 `search_conditions.target_journals`에는 저널 저명도 판단(위 두
  항목)만 동일하게 적용한다 - 검색 소스가 arXiv 등
  프리프린트 서버뿐이면 저널 저명도 개념이 없을 수 있으니, journal-site 소스를
  검색할 때만 이 규칙을 적용한다.

**Journal Watch의 후보 선별은 무작위로 한다** - `journals`가 채워진 팀에서 후보를 고를 때
**"이 위키의 기존 개념과 이어진다"를 가점으로 쓰지 않는다.** Journal Watch의 목적은 동향
감시인데, 기존 개념과의 연결을 기준으로 삼으면 위키가 자기강화되어 새로 떠오르는 주제가
구조적으로 누락된다. 기존 slug와 `wanted.md`를 제외한 신규 후보 풀을 만든 뒤 실제 난수로
(`python3 -c "import random; random.SystemRandom().sample(...)"`, `sort -R | head -n` 등 -
macOS에는 `shuf`가 없다) `papers_per_day`만큼 뽑고, 보고할 때 후보 풀 크기와 실제로 쓴
명령을 함께 밝혀 선별이 재량이 아니었음을 확인 가능하게 한다. 사용자가 특정 주제를
지정하면 그때만 그 주제로 좁힌다. Research Team은 반대다 - `topic`/`keywords`/
`my-research.md`에 맞춰 좁히는 것이 설계 의도이므로 기존 개념과 겹치는 것이 정상이다.
...
> [!important] **PDF 원본 없이는 위키 페이지를 쓰지 않는다** (아래 1~2a보다 우선)
> **논문 페이지(`papers/{slug}.md`)와 `raw/{slug}.json`은 `raw/{slug}.pdf`가 존재할 때만
> 작성한다.** 웹에서 읽은 전문, HTML 본문, 저장소 페이지, 프리프린트 전문 - 무엇이든
> **PDF 파일이 없으면 반영하지 않고 `wanted.md`로 보낸다.** 오픈 액세스라 읽을 수 있어
> 보여도 시도하지 않는다. 사용자가 PDF를 `raw/`에 넣고 `/paper-review-pdf`로 반영하는
> 경로로 일원화한다.
>
> **왜 이 규칙이 있는가** - 웹으로 읽고 작성한 논문 페이지들을 나중에 실제 PDF와 대조해
> 검증했더니 **절반 가까이에서 오류**가 나왔다. 회귀계수가 실제의 약 2배로 적히거나,
> 자료 연도가 통째로 틀리거나, 논문이 인용하지도 않은 문헌이 인용 목록에 올라가거나,
> 통계적으로 유의하지 않은 계수가 유의한 것처럼 서술되는 오류였다. **웹 경유 전문은
> 신뢰할 수 없다.**
>
> **작성 시 지켜야 할 것**
> - 반드시 `raw/{slug}.pdf`를 텍스트로 추출해(`pdftotext -layout` 등) 그 텍스트를 근거로 쓴다.
> - **숫자는 전부 원문 표/본문에서 확인한 값만 적는다.** 기억이나 추정으로 채우지 않는다.
>   확인하지 못한 값은 적지 말고 비워두거나 "논문 미명시"라고 쓴다.
> - **계수를 인용할 때는 유의수준(p값)을 함께 확인한다.** 비유의한 값을 유의한 것처럼 쓰지 않는다.
> - **"주요 인용 문헌"에는 그 논문이 실제로 인용한 문헌만 적는다.** 이론의 정본 출처를
>   안다고 해서 논문이 인용하지 않은 것을 적으면 안 된다.
> - 저자 해석("저자들은 ~라고 본다")은 원문에 실제로 그런 서술이 있을 때만 쓴다.
> - PDF가 발췌본·스캔 누락본이면 그 사실을 `wanted.md`에 "부분 반영"으로 남긴다.
>
> 아래 1·2a는 `/paper-review-pdf`(사용자가 넣은 PDF를 반영하는 경로)에서만 유효하다.
...
## PDF/Markdown 전문 직접 추가 시 (Full-text Ingest) 절차
...
전문은 `/paper-review-pdf <team-id>`로 반영한다. **사용자는 원문을 `.pdf`로 넣어도
되고, PDF에서 텍스트를 추출한 `.md`로 넣어도 된다** - 후자가 이미지 렌더링 비용이
없어 Read 토큰이 훨씬 적게 든다(표/수식/헤딩 구조가 살아있는 추출이어야 품질이
보장됨). 파일명은 둘 다 아무 이름이나 상관없다 - 표준 형식에 맞추는 건 이 절차가
대신한다.

1. `data/{team-path}/raw/`에서 확장자가 `.pdf` 또는 `.md`인 파일을 전부 확인한다
   (`my-research.md`처럼 팀 설정용으로 이미 존재하는 파일이 아니라, 이번에 사용자가
   전문 반영을 위해 새로 넣은 파일만 대상).
2. 각 파일을 Read 도구로 열어 제목/저자/연도/DOI 등을 원문에서 직접 확인한다.
...
3. 이 slug로 `raw/{slug}.json`이 이미 있고 `source_type`이 `full-text`인지
   확인한다. 이미 있으면 처리된 것이므로 건너뛴다(그 경우 원본 파일도 이미
   `raw/{slug}.pdf` 또는 `raw/{slug}.md`로 이름이 맞춰져 있을 것이다). 없으면
   "검토 대상 파일"로 판별한다.
4. 새로 판별된 파일 목록을 (원래 파일명 → 새로 정할 slug 매핑과 함께) 사용자에게
   보여주고 확인받는다.
5. 확인된 파일마다:
   - 파일명이 이미 `{slug}.pdf`/`{slug}.md`가 아니면 원래 확장자를 유지한 채
     `raw/{slug}.{pdf|md}`로 이름을 바꾼다(rename, 내용은 그대로 두고 파일명만
     변경).
...
시간이 지나 논문이 쌓이면 위키에 기계적인 정합성 문제가 누적된다: 나중에 추가된
논문/개념이 예전 논문의 "주요 인용 문헌"에 평문으로만 남거나, 여러 세션에 걸쳐 비슷한
태그(단수/복수, 하이픈 유무, 표기 차이)가 조금씩 다르게 생기거나, `papers/`에 반영된
논문이 `index.md` 목록에서 빠지는 식이다. `/wiki-review <team-id>`가 이들을 점검하고
**사용자 확인을 받은 뒤 실제로 고친다.**

**`/concept-review`와의 역할 구분 (중요)**: `/wiki-review`는 **이미 있는 페이지들의
기계적 정합성**만 다룬다 - 고칠 답이 하나로 정해지는 것들이다. **페이지를 새로 만들거나
없애거나 합치는 판단(개념·비교 페이지의 신설/병합/재구성/정의 갱신, 개념 간 관계 연결)은
전부 `/concept-review`가 맡는다.** 지식 구조에 대한 판단이라 deep-research 검증을 거쳐야
하기 때문이다. `/wiki-review`는 deep-research를 쓰지 않는다.

경계가 헷갈리면 **"고칠 답이 하나로 정해지는가?"**로 판단한다 - 그렇다면
`/wiki-review`(index에서 빠진 논문은 추가하는 것 외에 답이 없다), 아니라면
`/concept-review`(이 개념을 새로 만들지 말지는 판단의 문제다).

**점검만 하고 보고만 하는 항목** (자동으로 고치지 않음 - 사용자 판단 필요):
- 어떤 페이지에서도 링크되지 않는 "고아 페이지"가 있는가
- `concepts/` 페이지 간 서로 모순되는 설명이 있는가
- `raw/`에는 있는데 `papers/`에 대응 페이지가 없는 논문(역방향 누락) - 자동 생성하지
  않는다. 전문을 다시 읽어 정식으로 반영해야 하므로 `/paper-collect`나
  `/paper-review-pdf`로 유도한다.
- `synthesis/latest.md`가 최신 논문 추가 이후 갱신되지 않은 채로 오래됐는가
- (Research Team만) `advisor/latest.md`가 synthesis 갱신 이후 갱신되지 않았는가
- 검사 중 눈에 띈 개념·비교 페이지의 신설/병합 후보 - 고치지 않고 `/concept-review`로
  넘기라고 보고만 한다.

**점검 후 확인받아 실제로 고치는 항목**:
- `papers/`에는 있는데 `index.md`에는 반영 안 된 논문 → `index.md`에 추가.
- **인용/참조 백필링**: 논문 페이지의 "주요 인용 문헌"/"관련 논문"/"관련 개념"에
  평문으로만 적힌 인용이, 그 사이 위키에 실제로 추가된 논문·개념과 저자/제목/개념명이
  일치하면 `[[slug]]` 링크로 교체할 후보로 제안. 확실하지 않으면(저자 성만 같고 주제가
  다른 경우 등) 후보에서 제외 - 잘못된 링크보다 놓치는 편이 낫다.
- **태그 근접 중복 정리**: 팀 전체 논문의 `tags`를 모아, 같은 개념을 가리키는 근접
  중복(단수/복수, 하이픈 유무, 한/영 표기, 유사어)을 찾아 대표 표기로 통합할 안을 제안.
  애매하면(다른 개념일 수도 있으면) 묶지 않는다.

고치는 항목은 모두 **고치기 전에 후보 목록을 사용자에게 보여주고 전체/항목별/중단 중
확인받는다.** 정확한 절차는 `.claude/commands/wiki-review.md` 참고.

## Concept/Comparison 정리 절차 (`/concept-review <team-id>`)

`/paper-collect`와 `/paper-review-pdf`는 논문을 **한 편씩** 반영하면서 그 한 편만 놓고
"이 개념은 재사용 가능한가", "이 논문과 비교할 기존 논문이 있는가"를 판단한다. 그래서
**여러 논문에 걸쳐서만 드러나는 것**은 구조적으로 놓친다 - 세 편이 공유하는 공통
방법론이 개념으로 승격되지 못하거나, 같은 대상을 다르게 조작화한 논문 쌍이 비교 페이지로
묶이지 못한 채 남는다. 반대로 세션이 쌓이면 사실상 같은 개념/비교가 다른 slug로 중복
생성되기도 한다.

`/concept-review <team-id>`가 `concepts/`와 `comparisons/` 전체를 한 번에 훑어 이
사각지대를 다룬다. **페이지를 새로 만들거나 없애거나 합치는 판단은 전부 이 커맨드
소관이다**(`/wiki-review`와의 경계는 위 "Lint / 위키 정리 절차" 참고). 안전장치는
`/wiki-review`와 동일한 "먼저 목록 제시 → 전체/항목별/중단 확인 → 확인된 것만 수정"
패턴이다.

**기본은 증분 모드다.** `index.md`의 "최근 갱신" 로그에 이전 `/concept-review` 실행
기록이 있으면 그 회차 이후 추가된 논문만 본문까지 읽고, 나머지 논문은 프론트매터와
`관련 개념`/`관련 논문` 섹션만 읽는다. `concepts/`와 `comparisons/`는 병합·관계 판단에
전체 비교가 필요하므로 언제나 전부 읽는다. 신설 후보를 확정하기 전에는 그 논문의
`방법론` 섹션을 직접 읽어 확인한다 - 아래 사실 확인 규칙은 증분 모드에서도 예외가 없다.
첫 실행이거나 `--full` 인자가 붙으면 전체 모드로 돈다.

**deep-research 검증은 이 절차의 필수 단계다.** 이 커맨드의 판단은 전부 정답이 하나가
아닌 것들이라, 반론 검토를 거치지 않은 결론은 신뢰할 수 없다. 후보를 확정하기 전에
`deep-research`의 `devils_advocate_agent` CHECKPOINT 2(분석 후 검증)를 적용한다: 유리한
근거만 고르지 않았는가, **찾지 못한 증거는 무엇이고 그 부재가 결론을 바꾸는가**, 같은
근거의 다른 해석은 없는가, 포함 기준을 달리 잡으면 결론이 달라지는가, 이전에 "확신도
낮음"으로 보류한 건을 지금 통과시킨다면 근거가 새로 생긴 것인가 아니면 기준을 옮긴
것인가. 검증 결과는 후보 목록과 함께 사용자에게 보고한다.

**사실 확인 규칙**: 어떤 논문이 특정 방법론을 쓴다고 주장하려면 그 논문 페이지의
"방법론" 섹션과 프론트매터 `methods`를 직접 확인한다. 다른 개념 페이지의 서술을 근거로
삼지 않는다 - 개념 페이지의 틀린 서술을 근거로 새 개념 페이지를 만들고 역링크까지 달아
오류가 순환 증폭된 사례가 있다.

**점검 후 확인받아 실제로 고치는 항목**:
- **개념 페이지 신설**: 세 편 이상의 논문이 본문(방법론/데이터 및 변수/Discussion)이나
  `tags`에서 공통으로 다루는데도 `concepts/`에 대응 페이지가 없는 개념을 찾는다.
  "핵심/기초 개념 확장 규칙"(여러 논문 공통 전제 / 분야에서 정립된 개념 / 앞으로도 계속
  참조될 토대) 중 어디에 해당하는지와, **이 팀 연구와 실제로 닿는 지점**을 함께 밝힌다 -
  여러 논문이 공유한다는 사실만으로는 근거가 부족하다. 같은 단어가 우연히 겹치는 경우는
  제외한다.
- **비교 페이지 신설**: 같은 대상을 서로 다른 방식으로 조작화한 논문 쌍(또는 셋 이상) 중
  아직 비교 페이지가 없는 것을 찾는다. 조건은 (a) 겨냥하는 개념·현상이 같고, (b) 데이터·
  지표·분석 기법 중 최소 하나가 뚜렷이 갈리며, (c) 그 갈림이 이 팀의 방법론 선택에
  참고가 되는 것. (c)를 충족하지 못하면 만들지 않는다 - 비교를 위한 비교는 하지 않는다.
  셋 이상이 같은 축에서 갈리면 개별 쌍 대신 다자간 비교로 만든다.
- **개념 페이지 병합**: 제목/정의가 사실상 같은 대상을 가리키거나(동의어, 표기 차이)
  "관련 논문" 목록이 크게 겹치는 근접 중복 쌍을 찾는다. 확실한 경우만 병합하고, 관련은
  있지만 서로 다른 개념이면(포함 관계·인접 개념) 병합 대신 "관련 개념"으로 연결한다.
- **비교 페이지 병합/재구성**: 같은 대상 쌍을 다른 slug로 중복 작성한 경우를 병합한다.
  A vs B, A vs C처럼 한 대상을 축으로 흩어져 있으면 다자간 비교로 재구성한다.
- **개념 정의 갱신**: 어떤 개념 페이지의 "관련 논문"에는 없지만 이후 추가된 논문이
  본문에서 그 개념을 실제로 다루면, 그 논문을 반영해 정의/왜 중요한가/대표 방법론을
  확장한다(기존 내용을 지우지 않는다. 논문이 뒷받침하지 않는 내용은 지어내지 않는다).
- **개념 간 관계 연결**: 상위/하위 또는 인접 관계가 있는데 서로 링크되어 있지 않으면
  양쪽 "관련 개념" 섹션에 링크를 추가한다. 근거(공유 논문, 정의상의 포함/전제 관계)가
  분명할 때만 연결하고, 약하면 보고만 한다.
- **양방향 링크 정합성**: 논문 페이지의 "관련 개념"과 개념 페이지의 "관련 논문"이 서로를
  가리키는지 대조해 한쪽에만 있는 항목을 복구한다(판단이 필요 없는 기계적 검사).

**병합·재구성으로 사라지는 페이지는 삭제한다** - 사라지는 slug를 가리키던 위키 전체의
`[[링크]]`를 살아남는 slug로 먼저 일괄 치환하고, 잔여 참조가 0건인 것을 확인한 뒤 파일을
지운다. 위키 폴더는 git 버전관리 대상이 아니라 삭제가 되돌리기 어려우므로 **내용이
살아남는 페이지에 전부 흡수됐을 때만** 지운다.

**비교표에 `[[백링크]]`를 넣으려면 마크다운 표를 쓴다** - Obsidian은 `<table>` 같은 HTML
블록 안의 내용을 마크다운으로 처리하지 않아 `[[ ]]`가 링크가 아니라 문자 그대로 보인다.
열이 많아 표가 넓어지면 상단에 "비교 대상" 목록을 두어 거기에 `[[링크]]`를 모으고, 표
안에서는 짧은 별칭을 쓴 뒤 표를 주제별로 2개 이상으로 쪼갠다.
