...
**병합·재구성으로 사라지는 페이지는 실제로 지우지 않는다** - 위키 폴더는 git 버전관리
대상이 아니라 삭제가 되돌리기 어렵다. "이 {개념/비교}는 [[살아남는 slug]]로
통합되었습니다." 한 줄짜리 리다이렉트 스텁으로 바꾸고, 사라지는 slug를 가리키던 위키
전체의 `[[링크]]`를 살아남는 slug로 일괄 치환한다.
