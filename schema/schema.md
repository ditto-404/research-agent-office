...
- `raw/{slug}.json` (위치: `data/`) - 논문의 원본 메타데이터. API/웹에서 그대로
  ...
  참고. 사용자가 PDF를 직접 구해서 넣으면(같은 slug의 `raw/{slug}.pdf`)
...
PDF는 `/paper-review-pdf <team-id>`로 반영한다.

1. `data/{team-path}/raw/`에서 확장자가 `.pdf`인 파일을 전부 확인한다.
2. 각 PDF에 대해 파일명을 slug로 정규화(소문자, 하이픈)해 같은 slug의
   `raw/{slug}.json`이 이미 있고 `source_type`이 `full-text`인지 확인한다. 이미
   있으면 처리된 것이므로 건너뛴다. 없으면 "검토 대상 PDF"로 판별한다.
3. 새로 판별된 PDF 목록을 사용자에게 보여주고 확인받는다.
4. 확인된 PDF마다 Read 도구로 원문을 읽고 (제목/저자/연도/DOI 등은 원문에서 직접
   추출, 없으면 빈 값), `raw/{slug}.json`을 작성한다 - `source_api: "manual"`,
   `source_type: "full-text"`, `collected_at`은 오늘 날짜. 이 파일에도 해석은
   넣지 않는다 (사실만).
5. "Ingest 절차"의 2a(전문 확보 성공 경로)를 그대로 수행해 `wiki/papers/{slug}.md`
   등을 작성한다 - 실제로 원문에 근거가 있는 만큼만 서술하고, 원문에 없는 내용을
   지어내지 않는다.
6. `{wiki_path}/wanted.md`에 같은 논문으로 보이는 항목이 있으면 그 항목을
   제거한다 (정식으로 반영됐으므로).
